...
int wmain(int argc, WCHAR* argv[]) {
	...
	HANDLE hMapFileArmy2 = _wtoi(argv[2]);
	HANDLE hMapFileArmy1 = _wtoi(argv[3]);
	...
	own_troops = _wtoi(pBufferArmy2);
	enemy_troops = _wtoi(pBufferArmy1);
	ZeroMemory(buffer_own_troop, sizeof(buffer_own_troop));
	ZeroMemory(buffer_enemy_troop, sizeof(buffer_enemy_troop));
	_itow(own_troops, buffer_own_troop, 10);
	_itow(enemy_troops, buffer_enemy_troop, 10);
	CopyMemory(pBufferArmy2, buffer_own_troop, (wcslen(buffer_own_troop) * sizeof(WCHAR)));
	CopyMemory(pBufferArmy1, buffer_enemy_troop, (wcslen(buffer_enemy_troop) * sizeof(WCHAR)));
	//wprintf(L"Army2=%d enemy=%d\n", own_troops, enemy_troops);
	...
	else if (own_troops <= 0)wprintf(L"Army2 lose :(\n");
	else wprintf(L"Army2 error\n");
	return 0;
}